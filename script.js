...
// Builds a human-readable summary of a slot's not-yet-ended confirmed
// bookings, e.g. "This slot is already booked for 9:00 PM – 10:00 PM.
// Choose a different time to book it." Returns null if the slot has no
// upcoming/active bookings to mention. Shown when a slot is selected
// WITHOUT a time already chosen, so the person knows what to avoid before
// they fill in the booking form.
function getSlotBookingTimesInfo(slotId, floor) {
    const now = Date.now();
    ...
    const ranges = upcoming.map(b => {
        const start = new Date(b.startTime);
        const end = new Date(b.endTime);
        // Always include the date — a booking (especially an advance one)
        // can be for a different day than today, and showing only the time
        // was confusing/ambiguous.
        return `${formatDate(start)}, ${formatTime(start)} – ${formatTime(end)}`;
    });
    
    return `This slot is already booked for ${ranges.join(', ')}. Choose a different time to book it.`;
}
...
// ========================
// ========================
// Firebase-backed Parking Data
// ========================
// Replaces the old localStorage/random-generation version of this function.
// Slot layout (id, floor, type, price, distance) is deterministic and seeded
// into Firebase once. Slot STATUS (available/booked/occupied/reserved) lives
// entirely in Firebase and is kept in sync in real time via subscribeToParkingData().

const TOTAL_SLOTS = { ground: 120, first: 100, second: 80 };

function buildSlotLayout(floor) {
    const slots = [];
    for (let i = 1; i <= TOTAL_SLOTS[floor]; i++) {
...
// Local cache of the last known-good parking snapshot. Lets the UI paint
// real numbers INSTANTLY on page load (no "0 then wait" flash) while the
// live Firebase data streams in behind it and silently corrects anything
// that changed since the last visit.
const PARKING_CACHE_KEY = 'smartpark_parking_cache';
const SEEDED_CONFIRMED_KEY = 'smartpark_seeded_confirmed';

function loadCachedParkingSnapshot() {
    try {
        const cached = JSON.parse(localStorage.getItem(PARKING_CACHE_KEY));
        if (!cached) return;

        let hasData = false;
        ['ground', 'first', 'second'].forEach(floor => {
            if (Array.isArray(cached[floor]) && cached[floor].length > 0) {
                appState.parkingData[floor] = cached[floor];
                hasData = true;
            }
        });
        if (!hasData) return;
        ...
        console.error('Error loading cached parking snapshot:', e);
    }
}

function cacheParkingSnapshot() {
    try {
        localStorage.setItem(PARKING_CACHE_KEY, JSON.stringify(appState.parkingData));
    } catch (e) {
        // localStorage full/unavailable — non-critical, live data still works
    }
}

// Seeds Firebase with the slot layout ONLY if it hasn't been seeded before.
// Uses a meta/seeded flag so re-running this never overwrites live slot statuses.
async function seedParkingDataIfNeeded() {
    const { db, ref, get, set } = window.SmartParkFirebase;
    const seededRef = ref(db, 'meta/seeded');
    const snapshot = await get(seededRef);

    if (snapshot.exists() && snapshot.val() === true) {
        console.log('Firebase parking data already seeded, skipping.');
        return;
    }

    console.log('Seeding Firebase with initial parking layout...');
    const floors = ['ground', 'first', 'second'];
    for (const floor of floors) {
        const slots = buildSlotLayout(floor);
        const floorData = {};
        slots.forEach(slot => { floorData[slot.id] = slot; });
        await set(ref(db, `parking/${floor}`), floorData);
    }
    await set(seededRef, true);
    console.log('Firebase seeding complete.');
}

// Attaches a real-time listener per floor. Whenever ANY client changes a
// slot's status (e.g. books it), every connected client's appState updates
// automatically and the map/stats re-render.
function subscribeToParkingData() {
    ...
    floors.forEach(floor => {
        ...
        onValue(floorRef, (snapshot) => {
            ...
            appState.parkingData[floor] = Object.values(data);
            cacheParkingSnapshot();
...
// Kicks off the Firebase-backed parking data flow. Paints instantly from the
// local cache (if any), then confirms/refines with live Firebase data.
// On repeat visits, skips the "is it seeded?" network round-trip entirely
// once we've confirmed it once before — that round-trip was pure overhead
// after the very first load and was a big chunk of the perceived delay.
function generateParkingData() {
    loadCachedParkingSnapshot();

    function start() {
        if (localStorage.getItem(SEEDED_CONFIRMED_KEY) === 'true') {
            subscribeToParkingData();
            return;
        }

        seedParkingDataIfNeeded().then(() => {
            localStorage.setItem(SEEDED_CONFIRMED_KEY, 'true');
            subscribeToParkingData();
        }).catch(err => console.error('Firebase parking init error:', err));
    }

    if (window.SmartParkFirebase) {
        start();
    } else {
        document.addEventListener('firebase:ready', start, { once: true });
    }
}
...
function renderParkingMap() {
    ...
    // While browsing for a specific advance-booking time window, colors
    // must reflect availability for THAT window, not each slot's raw
    // persisted status — otherwise a slot booked for 11pm would still look
    // (and act, before the fix above) unavailable at 6pm, even though it's
    // genuinely free then.
    let advanceWindow = null;
    if (appState.advanceBooking) {
        advanceWindow = {
            start: new Date(appState.advanceBooking.startTime),
            end: new Date(appState.advanceBooking.endTime)
        };
    }
    ...
    slots.forEach(slot => {
        ...
        // Always compute the slot's TRUE status live from actual booking
        // data, instead of trusting the persisted status field — that
        // field only gets re-synced to Firebase periodically (every 30s)
        // or on specific triggers, so it can briefly lag right around a
        // booking's start time. Computing live here means the map is
        // always correct regardless of that sync timing.
        let displayStatus = computeAuthoritativeSlotStatus(slot.floor, slot.id);
        
        if (advanceWindow) {
            // While browsing for a specific advance-booking time window,
            // colors must reflect availability for THAT window instead —
            // a slot booked for 11pm should still look available at 6pm.
            const freeForRequestedWindow = isSlotAvailableForBooking(
                slot.id, slot.floor, advanceWindow.start, advanceWindow.end
            );
            displayStatus = freeForRequestedWindow ? 'available' : 'booked';
        }
        
        slotElement.className = `parking-slot-map ${displayStatus} ${slot.type}`;
...
function selectParkingSlot(slot) {
    // Selecting a slot on the map never blocks purely on its blanket
    // status color anymore. That color only means "this slot has SOME
    // booking somewhere" — it doesn't know what time the person actually
    // wants yet. The real conflict check only makes sense once an actual
    // start/end time is known:
    //   - if they came through "Book in Advance", that time is already
    //     chosen, so we can check it right now.
    //   - otherwise, the booking form (opened below) is where they pick a
    //     time — so we let them in, tell them what's already booked on
    //     this slot so they know what to avoid, and the true enforcement
    //     happens at "Confirm & Pay" via isSlotAvailableForBooking(),
    //     which already checks the SPECIFIC time they chose.
    if (appState.advanceBooking) {
        ...
        if (!isSlotAvailableForBooking(slot.id, slot.floor, startTime, endTime)) {
            showToast('This slot is already booked for your selected time', 'error');
            return;
        }
    } else {
        const bookedTimesInfo = getSlotBookingTimesInfo(slot.id, slot.floor);
        if (bookedTimesInfo) {
            showToast(bookedTimesInfo, 'info');
        } else if (slot.status !== 'available') {
            // A non-available slot with no booking behind it shouldn't
            // normally happen (the background sweep self-heals this within
            // 30s), but if this fires between one moment and the next
            // sweep tick, still give clear feedback instead of silently
            // doing nothing.
            showToast('This slot is currently unavailable. It should free up shortly — try Refresh.', 'info');
        }
    }
...
function updateSlotInfoPanel(slot) {
    ...
    // Live-computed, same as the map and floor stats — the raw persisted
    // slot.status field can lag until the next background sync.
    const liveStatus = computeAuthoritativeSlotStatus(slot.floor, slot.id);
    ...
    document.getElementById('selected-slot-status').className = `status-${liveStatus}`;
...
function updateFloorStatistics() {
    ...
    // Computed live (same reasoning as renderParkingMap) so this panel
    // can't show a stale "100% available" right after a booking's start
    // time arrives, before the periodic sync catches up.
    let available = 0;
    slots.forEach(s => {
        if (computeAuthoritativeSlotStatus(s.floor, s.id) === 'available') available++;
    });
    const notAvailable = total - available; // reserved + booked + occupied, combined
    ...
    if (bookedEl) bookedEl.textContent = notAvailable;
...
function updateAvailableSlotsCount() {
    ...
    Object.values(appState.parkingData).forEach(floor => {
        floor.forEach(s => {
            if (computeAuthoritativeSlotStatus(s.floor, s.id) === 'available') totalAvailable++;
        });
        totalEV += floor.filter(s => s.type === 'electric').length;
    });
    ...
    const evSpotsElement = document.getElementById('quick-ev-spots');
    ...
    if (evSpotsElement) evSpotsElement.textContent = totalEV;
    
    animateCounter(quickSlotsElement, totalAvailable);
    animateCounter(evSpotsElement, totalEV);
}

// ========================
// Real Avg. Search Time Tracking
// ========================
// A "search" starts the moment the user scrolls into the Live Parking
// section (see setupSearchTimeTracking) and ends the moment they pick an
// available slot. Durations are kept in localStorage so the stat reflects
// real usage across visits instead of a hardcoded number.
//
// CONCEPT: this measures search RESPONSIVENESS — the time from the user
// triggering a search (clicking "Find Parking", or switching floors to see
// that floor's availability) to the results actually being visible on
// screen. It intentionally does NOT measure how long the user then spends
// deciding which slot to book — that's human browsing behavior, not search
// speed, and mixing the two produced misleadingly large/unstable numbers.

const SEARCH_TIME_STORAGE_KEY = 'smartpark_search_times';
const MAX_SEARCH_TIME_SAMPLES = 100;
const MAX_REASONABLE_SEARCH_SECONDS = 30; // anything past this isn't "search time" anymore

function getSearchTimeSamples() {
    try {
        return JSON.parse(localStorage.getItem(SEARCH_TIME_STORAGE_KEY)) || [];
    } catch (e) {
        return [];
    }
}

function recordSearchTime(seconds) {
    if (!isFinite(seconds) || seconds <= 0 || seconds > MAX_REASONABLE_SEARCH_SECONDS) return;
    
    const samples = getSearchTimeSamples();
    samples.push(Math.round(seconds * 10) / 10);
    
    while (samples.length > MAX_SEARCH_TIME_SAMPLES) samples.shift();
    
    localStorage.setItem(SEARCH_TIME_STORAGE_KEY, JSON.stringify(samples));
    updateAvgSearchTimeStat();
}

function getAvgSearchTime() {
    const samples = getSearchTimeSamples();
...
function updateAvgSearchTimeStat() {
    const el = document.getElementById('quick-avg-search-time');
    if (!el) return;
    
    const avg = getAvgSearchTime();
    ...
    // Set directly — no animateCounter() here. animateCounter() reads the
    // current value with parseInt(), which truncates decimals (e.g. "10.4"
    // becomes 10), so it would think there's a 0.4 "difference" to animate
    // and run a needless ~1s roll-up that shows a rounded, wrong number the
    // whole time before finally snapping to the real value. This stat is
    // usually a decimal, so it must be set directly to display instantly.
    el.textContent = avg;
}
...
function stopSearchTimerAndRecord() {
    if (appState.searchStartTime) {
        const elapsedSeconds = (Date.now() - appState.searchStartTime) / 1000;
        recordSearchTime(elapsedSeconds);
        appState.searchStartTime = null;
    }
}

// After a "Find Parking" click triggers a smooth scroll to the live map,
// the search isn't "done" until the scroll physically stops and the
// results are actually on screen — so we watch for scroll position to
// settle (no movement for ~6 animation frames, ~100ms) before recording.
function watchForScrollSettle() {
    let lastY = window.scrollY;
    let stableFrames = 0;
    const REQUIRED_STABLE_FRAMES = 6;
    const startedAt = Date.now();
    const MAX_WAIT_MS = 3000; // safety net if scroll never settles
    ...
    function check() {
        ...
        if (stableFrames >= REQUIRED_STABLE_FRAMES || Date.now() - startedAt > MAX_WAIT_MS) {
            stopSearchTimerAndRecord();
            return;
        }
...
function setupSearchTimeTracking() {
    updateAvgSearchTimeStat();
}

// ========================
// Real User Satisfaction Tracking
// ========================
// Collected via a 5-star prompt shown on the booking success modal.
// Score shown in the hero stats is (average rating / 5) * 100.

const SATISFACTION_STORAGE_KEY = 'smartpark_satisfaction_ratings';
const MAX_SATISFACTION_SAMPLES = 200;

function getSatisfactionSamples() {
    try {
        return JSON.parse(localStorage.getItem(SATISFACTION_STORAGE_KEY)) || [];
    } catch (e) {
        return [];
    }
}

function recordSatisfactionRating(rating) {
    if (!Number.isInteger(rating) || rating < 1 || rating > 5) return;
    
    const samples = getSatisfactionSamples();
    samples.push(rating);
    
    while (samples.length > MAX_SATISFACTION_SAMPLES) samples.shift();
    
    localStorage.setItem(SATISFACTION_STORAGE_KEY, JSON.stringify(samples));
    updateSatisfactionStat();
}
...
function getSatisfactionScore() {
    ...
    if (samples.length === 0) return null;
    const sum = samples.reduce((a, b) => a + b, 0);
    return Math.round((sum / samples.length / 5) * 100);
}

function updateSatisfactionStat() {
    const el = document.getElementById('quick-user-satisfaction');
    if (!el) return;
    
    const score = getSatisfactionScore();
    
    if (score === null) {
        // No ratings yet — "0" would read as "0% satisfied", which is
        // misleading. Show "no data yet" instead of a fabricated number.
        el.textContent = '–';
        return;
    }
...
function setupSatisfactionRating() {
    updateSatisfactionStat();
    ...
    stars.forEach(star => {
        star.addEventListener('mouseenter', () => {
            const hoverValue = parseInt(star.dataset.rating, 10);
            stars.forEach(s => {
                s.classList.toggle('hovered', parseInt(s.dataset.rating, 10) <= hoverValue);
            });
        });
        ...
        star.addEventListener('click', () => {
            ...
            stars.forEach(s => {
                const isFilled = parseInt(s.dataset.rating, 10) <= rating;
                s.classList.toggle('fas', isFilled);
                s.classList.toggle('far', !isFilled);
            });
            
            recordSatisfactionRating(rating);
...
function resetSatisfactionRatingWidget() {
    const starsContainer = document.getElementById('rating-stars');
    const thanksEl = document.getElementById('rating-thanks');
    if (!starsContainer) return;
...
function setupParkingFeatures() {
    ...
    if (refreshBtn) {
        refreshBtn.addEventListener('click', function() {
            showLoading();
            setTimeout(() => {
                // Reserved/Booked/Occupied all auto-correct on a 30s
                // background schedule (see refreshBookingDrivenSlotStatuses)
                // — a manual refresh just forces that check to run
                // immediately, which is most noticeable right around a
                // booking's start or end time.
...
function setupBooking() {
    // A native time picker's scroll wheel can't be restricted to only
    // show valid hours/minutes — that's a browser platform limitation,
    // not something CSS/HTML min attributes can fully control. What we
    // CAN do is reject an invalid pick immediately, the moment it's made,
    // instead of only catching it later when "Next" is clicked.
    const bookingTimeInput = document.getElementById('booking-time');
    if (bookingTimeInput) {
        bookingTimeInput.addEventListener('change', function() {
            if (this.min && this.value && this.value < this.min) {
                showToast('That time has already passed — please pick a current or later time', 'error');
                this.value = '';
            }
        });
    }
...
function setupFAQAccordion() {
    document.querySelectorAll('.faq-container').forEach(container => {
        container.addEventListener('click', (e) => {
            ...
            // The CSS (see "FAQ ACCORDION - SMOOTH OPENING & CLOSING FIX"
            // in style.css) drives the entire open/close animation AND the
            // chevron icon rotation purely off .faq-item's "active" class —
            // so that's the ONLY thing this handler needs to toggle. No
            // separate icon class swapping or .faq-answer class toggling
            // needed; that was leftover from an earlier version and no
            // longer matches how the CSS actually works, which is what
            // caused the two handlers to fight each other and need two
            // clicks to close.
...
// project already used for live parking data). Requires Google to be
// turned on as a Sign-in provider in Firebase Console -> Authentication ->
// Sign-in method, and the site's domain to be in Authorized domains there —
// that's a one-time setup step on the Firebase project itself, not
// something fixable from this code alone.
function handleGoogleLogin() {
    if (!window.SmartParkFirebase || !window.SmartParkFirebase.auth || !window.SmartParkFirebase.signInWithPopup) {
    ...
    signInWithPopup(auth, googleProvider)
        ...
        .catch((error) => {
            ...
            if (error.code === 'auth/popup-closed-by-user' || error.code === 'auth/cancelled-popup-request') {
                return; // user closed the popup themselves — no error needed
            }
            
            if (error.code === 'auth/operation-not-allowed') {
                showToast('Google sign-in is not enabled for this project yet', 'error');
...
function validateBookingStep1() {
    ...
    const dateString = document.getElementById('booking-date').value; // dd-mm-yyyy
    ...
    // Use the ACTUAL selected date, not always "today" — the previous
    // version always compared against today's date even for an advance
    // booking on a future day, which could wrongly reject a perfectly
    // valid future booking (e.g. tomorrow 10am while it's currently 4pm
    // today looked like "the past" under the old, date-blind comparison).
    const now = new Date();
    const [day, month, year] = dateString.split('-');
    const selectedDate = new Date(`${year}-${month}-${day}T${time}`);
...
function formatISODateToDDMMYYYY(isoDate) {
    // appState.advanceBooking.date comes from a native <input type="date">,
    // which always uses yyyy-mm-dd. The main booking form's #booking-date
    // field is a plain text field that everywhere else in this file expects
    // dd-mm-yyyy. Without this conversion, the advance-booking flow silently
    // produces an Invalid Date a few steps later, and Invalid Date.toISOString()
    // throws — which happens INSIDE the payment setTimeout callback, so the
    // hideLoading() call right after it never runs and the modal is stuck on
    // "Loading..." forever. This was the exact bug reported.
    const [year, month, day] = isoDate.split('-');
    return `${day}-${month}-${year}`;
}

// Prevents picking a past time in #booking-time whenever the booking date
// (dd-mm-yyyy in #booking-date) is today. Future dates (from an advance
// booking) get no restriction — any time on a future day is fine.
function applyBookingTimeMinConstraint() {
    ...
    if (isToday) {
        const nowPlusOneMin = new Date(Date.now() + 60000);
        const hh = String(nowPlusOneMin.getHours()).padStart(2, '0');
        const mm = String(nowPlusOneMin.getMinutes()).padStart(2, '0');
        timeInput.min = `${hh}:${mm}`;
...
function updateBookingFormWithSlot(slot) {
    ...
    applyBookingTimeMinConstraint();
    
    if (appState.advanceBooking) {
        document.getElementById('booking-date').value = formatISODateToDDMMYYYY(appState.advanceBooking.date);
...
function handlePayment() {
    ...
    // Check if slot is available for the SPECIFIC time window chosen —
    // not whether the slot has any booking at all.
    if (!isSlotAvailableForBooking(appState.selectedSlot.id, appState.currentFloor, startTime, endTime)) {
        const bookedTimesInfo = getSlotBookingTimesInfo(appState.selectedSlot.id, appState.currentFloor);
        showToast(bookedTimesInfo || 'This slot is already booked for the selected time', 'error');
        return;
    }
    ...
    setTimeout(() => {
        ...
        // booking for a different, non-overlapping time.
...
// non-overlapping time windows (e.g. booked 6-7pm AND separately 11pm-12am).
// The slot's one persisted `status` field has to summarize all of them at
// once, using a purely time-based rule (the SAME rule regardless of which
// flow created the booking, since both "Book Now" and "Book in Advance"
// let a person choose any time — there's no such thing as a booking that's
// exempt from the clock):
//   - OCCUPIED (red):    some booking's window contains THIS EXACT MOMENT
//   - RESERVED (blue):   not active now, but the soonest upcoming booking
//                         for this slot came through "Book in Advance"
//   - BOOKED (yellow):   not active now, but the soonest upcoming booking
...
// booking) and corrects its status to whatever computeAuthoritativeSlotStatus
// says it should genuinely be right now. This is what makes
// occupied/reserved/booked real, live statuses instead of something only
// set once and never revisited — and, because it checks every slot rather
// than only ones with a live booking record, it also self-heals any slot
// left stuck in a stale status with no booking behind it at all (e.g. from
...
function refreshBookingDrivenSlotStatuses() {
    ...
    Object.keys(appState.parkingData).forEach(floor => {
        ...
        floorSlots.forEach(slot => {
            // Only these three are booking-driven; 'available' never needs
            // correcting.
            if (slot.status !== 'reserved' && slot.status !== 'booked' && slot.status !== 'occupied') return;
...
// appState.selectedSlot/appState.currentFloor) so this works correctly for
// BOTH a fresh booking AND cancelling/releasing a past booking for a slot
// that may not even be the one currently selected on the map.
function updateParkingAfterBooking(floor, slotId, status) {
    if (floor && slotId && status) {
        // Write the status change to Firebase. We do NOT mutate
        // appState.parkingData directly here anymore — the onValue
        // listener from subscribeToParkingData() will receive this
    ...
    // Kept as an immediate optimistic UI update so the current user sees
    // feedback instantly, without waiting for the round-trip. The Firebase
    // listener will reconcile shortly after with the authoritative state.
    renderParkingMap();
    updateFloorStatistics();
    updateAvailableSlotsCount();
...
function updateDashboard() {
    // No early return for logged-out state: the stat cards, sidebar counts,
    // and recent-activity list must be actively reset to their guest
    // defaults on logout, not just left showing the previous user's numbers
    // until a page refresh happens to rebuild them from scratch.
...
function updateDashboardOverview() {
    ...
    if (appState.isLoggedIn && appState.currentUser) {
        ...
        totalHours = userBookings.reduce((total, booking) => total + booking.duration, 0);
        totalSpent = userBookings.reduce((total, booking) => total + booking.total, 0);
...
function logout() {
    ...
    appState.planDetails = null;
    appState.advanceBooking = null;
    // Clear the in-memory bookings/payments too, not just the logged-in user —
    // otherwise the previous account's bookings stay visible after logout
    ...
    updateUserUI();
    updateDashboard();
    // Same reason as on login: bookings changed in appState, so the map
    // needs an explicit re-render or it keeps showing the previous user's
    // slot color until something else happens to redraw it.
    renderParkingMap();
    updateFloorStatistics();
...
function updateBookingFormWithPlan(plan) {
    ...
    planInfoElement.innerHTML = `
        <div style="display: flex; align-items: center; gap: 0.75rem; margin-bottom: 0.5rem;">
            ...
            <h4 style="margin: 0; font-size: 1.125rem; color: white;">${planDetails.name} Plan Selected</h4>
        </div>
        <div style="display: flex; justify-content: space-between; align-items: center;">
            <div>
                <p style="margin: 0.25rem 0; font-size: 0.875rem; opacity: 0.9; color: white;">
                    <i class="fas fa-check-circle"></i> ${planDetails.price}${planDetails.period}
                </p>
                ${planDetails.features.slice(0, 2).map(feature => `
                    <p style="margin: 0.25rem 0; font-size: 0.75rem; opacity: 0.8; color: white;">
...
function openBookInAdvanceModal() {
    ...
    if (dateInput && timeInput) {
        ...
        // Keeps the time picker from allowing a past time whenever "today"
        // is the selected date — re-checked every time the date changes,
        // since "today" itself can only be picked or deselected by hand.
        const updateAdvanceTimeMinConstraint = () => {
        ...
        dateInput.addEventListener('change', updateAdvanceTimeMinConstraint);
        timeInput.addEventListener('change', updateSummary);
        timeInput.addEventListener('change', function() {
            // Same reasoning as the main booking form: the native picker's
            // scroll wheel can still show past times even with min set, so
            // reject an invalid pick immediately rather than waiting for
            // "Select Parking Slot" to be clicked.
            if (this.min && this.value && this.value < this.min) {
                showToast('That time has already passed — please pick a current or later time', 'error');
                this.value = this.min;
                updateSummary();
        ...
        updateAdvanceTimeMinConstraint();
        updateSummary();
    }
...
// NOTE: Slot status updates are now pushed in real time by Firebase via
// subscribeToParkingData() (see generateParkingData() above) — there is no
// more randomly-generated fake activity. This function is kept only as a
// lightweight UI heartbeat for the "Updated at HH:MM:SS" label and the
// header slot counter, in case no Firebase write happens for a while.
function startLiveUpdates() {
    let statsInterval;
    let statusSweepInterval;
    ...
    // Keeps "Reserved" and "Booked" honest as real time passes — an advance
    // booking automatically becomes "Booked" once its start time arrives,
    // and any booking automatically releases its slot back to "Available"
    // once its paid time window ends. Runs even if nobody touches that slot.
    refreshBookingDrivenSlotStatuses();
    statusSweepInterval = setInterval(refreshBookingDrivenSlotStatuses, 30000);

    window.addEventListener('beforeunload', () => {
        clearInterval(statsInterval);
        clearInterval(statusSweepInterval);
    });
}
...
function cancelBooking(bookingId) {
    if (confirm('Are you sure you want to cancel this booking?')) {
        ...
        if (bookingIndex !== -1) {
            ...
            updateSlotStatus(booking.slotId, booking.floor, slotStatusAfterCancel);
