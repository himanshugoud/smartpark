    ...
    const waitForDashboard = setInterval(() => {
        ...
        if (dashboardNav) {
            ...
            if (!document.querySelector('[data-panel="vehicles"]')) {
                ...
                // Insert before "Settings" so the order is
                // Overview / My Bookings / Payments / My Vehicles / Settings
                // instead of always landing at the very end.
                const settingsTab = dashboardNav.querySelector('[data-panel="settings"]');
                if (settingsTab) {
                    dashboardNav.insertBefore(vehiclesTab, settingsTab);
                } else {
