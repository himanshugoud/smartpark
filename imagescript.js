...
function setupProfileImageUpload() {
    ...
    // NOTE: this decision is made further down, AFTER profileImage is
    // actually attached to the document — createDefaultAvatar() looks
    // itself up via document.getElementById, which finds nothing (and
    // silently no-ops, leaving a blank circle) if the element only exists
    // in memory and hasn't been appended to the page yet.
    ...
    // Now that profileImage is actually in the document, it's safe to
    // generate the initials avatar (it looks itself up via getElementById).
    if (!savedImage) {
        createDefaultAvatar();
    }

    // Reflect whatever's already saved (or not) onto the nav icon right away,
    // since this setup runs on a delay after page load and updateUserUI()'s
    // earlier call to updateProfileImage() would have found #profile-image
    // missing and skipped the nav sync entirely.
    syncNavAccountAvatar();
...
function showImageOptionsModal() {
    ...
    if (hasImage) {
        modalContent.innerHTML = `
            <h3 style="margin-bottom: 0.6rem; font-size: 1.1rem; color: var(--gray-900);">
                <i class="fas fa-image"></i> Profile Image Options
            </h3>
            <p style="margin-bottom: 1rem; color: var(--gray-600); font-size: 0.8125rem;">
    ...
    style.textContent = `
        ...
        #change-image-btn:hover, #upload-image-btn:hover {
            ...
            /* .btn-outline:hover normally also sets color:white, assuming
               the background inverts to solid blue. Here the background is
               just a subtle translucent tint (by design), so the text/icon
               must stay blue instead of inheriting that white — otherwise
               it's white text on a barely-tinted background, unreadable. */
            color: var(--primary);
        }
...
function handleImageUpload(event) {
    ...
    reader.onload = function(e) {
        ...
        // Reflect the new photo on the small nav "Account" icon immediately,
        // instead of waiting for the next login/logout to sync it.
        syncNavAccountAvatar();
...
function deleteProfileImage() {
    if (confirm('Are you sure you want to delete your profile image?')) {
        ...
        // Revert the small nav "Account" icon back to the default right away.
        syncNavAccountAvatar();
...
// Mirrors the actual uploaded photo (not the generated-initials avatar) onto
// the small "Account" icon in the top nav, so it always matches what's on
// the dashboard profile card. Falls back to the default user icon whenever
// there's no logged-in user or no uploaded photo — including right after
// deleteProfileImage() removes it.
function syncNavAccountAvatar() {
    const navIcon = document.getElementById('nav-account-icon');
    const navAvatar = document.getElementById('nav-account-avatar');
    if (!navIcon || !navAvatar) return;

    const savedImage = (typeof appState !== 'undefined' && appState.isLoggedIn)
        ? localStorage.getItem('smartpark_profile_image')
        : null;
...
window.syncNavAccountAvatar = syncNavAccountAvatar;
