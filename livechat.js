...
class LiveChatSystem {
    constructor() {
        ...
        this.botResponses = {
            ...
            // ===== ABOUT SMARTPARK - GENERAL SITE INFO =====
            'about_smartpark': [
                "SmartPark is a real-time parking management system with 300 slots across 3 floors (120 Ground, 100 First, 80 Second). You can check live availability, book a slot, pay online, and manage everything from your dashboard!",
                "SmartPark helps you find and book parking instantly. We track 300 slots across 3 floors in real time, with dynamic pricing, EV charging spots, and secure online payments.",
                "This is SmartPark - a smart parking platform covering 3 floors and 300 total slots. Live availability updates automatically across all users, so you always see accurate spot status.",
                "SmartPark lets you view live slot availability, book in a few clicks, pay online, and track your booking history - all across our 3-floor, 300-slot facility."
            ],

            'thanks': [
                "You're welcome! Let me know if you need anything else.",
                "Anytime! Happy to help with your parking needs.",
                "No problem at all! Feel free to ask if anything else comes up.",
                "Glad I could help! I'm here if you need more assistance."
            ],

            'goodbye': [
                "Take care! Come back anytime you need parking help.",
                "Bye! Have a great day, and happy parking!",
                "See you soon! I'll be here whenever you need assistance.",
                "Goodbye! Drive safe and park smart!"
            ],
            ...
            'founder_story': [
                "Himanshu Goud started SmartPark as a personal project while pursuing B.Tech in Mathematics & Computing at MITS Gwalior. He built it from the ground up - UI, booking engine, and later added Firebase Realtime Database so parking availability syncs live across every user, in real time!",
                "The story: Himanshu Goud, a MITS Gwalior student, saw how frustrating parking search could be and built SmartPark to fix it - complete with live slot tracking across 3 floors and 300 spots, powered by Firebase Realtime Database.",
                "Himanshu Goud designed and coded SmartPark end-to-end - from the booking flow to real-time synchronization via Firebase, so any booking you make updates instantly for every other visitor too!",
                "Founder Himanshu Goud built SmartPark to solve real parking chaos - starting with a static prototype and evolving it into a fully real-time system using Firebase Realtime Database, so slot status stays accurate for everyone, live.",
                "SmartPark began as Himanshu Goud's B.Tech project at MITS Gwalior (Mathematics & Computing). What started as a UI concept is now a real-time, multi-user parking platform built on Firebase!"
            ],
            
            // ===== GENERAL BOOKING / PRICING OVERVIEW =====
            'how_to_book_general': [
                "Booking is easy: 1) Go to Live Parking and pick a floor 2) Click an available (green) slot 3) Enter your vehicle details and duration 4) Pay online. You'll get instant confirmation!",
                "To book a spot: select a floor, click an open slot, fill in your vehicle info and time, then complete payment. Takes less than a minute!",
                "Just head to the Live Parking section, choose a slot, add your vehicle details and duration, and pay - your spot is reserved instantly.",
                "Booking steps: Find a slot → Select it → Add vehicle & duration → Pay → Done! You'll see your booking in the Dashboard right after."
            ],

            'pricing_general': [
                "Our pricing: Standard $2.50/hr, Premium $3.50/hr (covered, closer to exits), or Business $180/month for unlimited access. EVs get 15% off!",
                "We have 3 plans - Standard ($2.50/hr), Premium ($3.50/hr), and Business ($180/mo unlimited). Peak hours (9AM-5PM weekdays) add a 20% surcharge.",
                "Pricing starts at $2.50/hr for Standard spots, $3.50/hr for Premium, or $180/month for Business. Weekend flat rate is $12/day, and early birds (before 8AM) pay just $15 for the whole day!",
                "Rates: Standard $2.50/hr, Premium $3.50/hr, Business $180/mo. Night rate (8PM-6AM) is a flat $8, and EVs always get 15% off."
            ],
            'how_to_cancel': [
            ...
            'support_general': [
                "Need help? Call us 24/7 at +91 9575228807, or email himanshugoud638@gmail.com (2hr response). Our office is open 8AM-8PM daily too!",
                "We've got a few support options: phone (+91 9575228807, 24/7), email (himanshugoud638@gmail.com), or visit our office 8AM-8PM daily.",
                "Support is available around the clock by phone at +91 9575228807, or by email at himanshugoud638@gmail.com if it's non-urgent.",
                "You can reach support via phone (+91 9575228807, anytime), email (himanshugoud638@gmail.com), or in person 8AM-8PM daily."
            ],
        ...
        this.keywordMap = {
            ...
            'available_now': [
                ...
                'how many spots', 'vacant', 'empty spots', 'available spot',
                'spot available', 'any spot', 'free spot', 'empty spot',
                'open spot', 'open spots', 'available slot', 'slot available'
            ],
            'ground_floor': [
    ...
    toggleChat() {
        ...
        if (this.isOpen) {
            ...
            // Swap to a close (X) icon instead of rotating the speech-bubble
            // icon 90° — rotating the same bubble shape doesn't read as a
            // "close" affordance, it just looks like a tilted bubble.
            if (toggleIcon) {
    ...
    determineResponseType(message) {
        ...
        // Normalize: strip punctuation/extra whitespace so "Hii!!", "hello!", 
        // "  hey  " etc. all match correctly instead of only an exact "hi".
        const normalizedMsg = lowerMsg.replace(/[^\w\s']/g, '').trim();
        
        // ---- Helper: word-boundary contains-any check, used throughout the
        // topic detection below so word ORDER/exact phrasing doesn't matter
        // (e.g. "ev charging rate" and "rate for ev" both work the same way),
        // and so "cancel" doesn't false-match inside unrelated words.
        const has = (w) => new RegExp(`\\b${w}\\b`, 'i').test(lowerMsg);
        const hasAny = (arr) => arr.some(has);
        const priceWords = ['price', 'prices', 'rate', 'rates', 'cost', 'costs', 'pricing', 'fee', 'fees'];
        const cancelWords = ['cancel', 'cancellation', 'cancelling'];

        // ===== GREETING DETECTION =====
        // Matches common greetings even with repeated letters (hii, heyy, hellooo)
        // or typos, as long as the message is short and greeting-like.
        if (/^(h+i+|h+e+y+a?|h+e+l+l+o+|hola|namaste|yo+|greetings?)$/i.test(normalizedMsg) ||
            /^(h+i+|h+e+y+|h+e+l+l+o+)\s+(there|parky|bot|smartpark)$/i.test(normalizedMsg)) {
            return 'greeting';
        }
        ...
        // ===== ABOUT SMARTPARK DETECTION =====
        if (lowerMsg.includes('what is smartpark') || lowerMsg.includes('what is this') ||
            lowerMsg.includes('about smartpark') ||
            (lowerMsg.includes('tell me about') && !lowerMsg.includes('founder') && !lowerMsg.includes('creator') && !lowerMsg.includes('developer')) ||
            lowerMsg.includes('what does this') || lowerMsg.includes('what does smartpark') ||
            lowerMsg.includes('how many slots') || lowerMsg.includes('total slots') ||
            lowerMsg.includes('how many floors') || lowerMsg.includes('how many spots total')) {
            return 'about_smartpark';
        }
        
        // ===== THANKS / GOODBYE DETECTION =====
        if (/^(thanks|thank you|thankyou|thnx|ty|tysm)[\s!.]*$/i.test(lowerMsg.trim())) {
            return 'thanks';
        }
        if (/^(bye|goodbye|see ya|see you|cya|later|gtg)[\s!.]*$/i.test(lowerMsg.trim())) {
            return 'goodbye';
        }
        
        // ===== FOUNDER DETECTION =====
        if (has('founder') && hasAny(['contact', 'reach', 'email'])) {
            return 'contact_founder';
        }
        if (has('founder') && hasAny(['story', 'how started', 'beginning'])) {
            return 'founder_story';
        }
        if ((has('how') && has('start') && has('smartpark')) || has('beginning of smartpark')) {
            return 'founder_story';
        }
        if (has('who') && has('founder')) {
            return 'who_is_founder';
        }
        // Catch-all for any other founder/creator/developer mention that
        // didn't match the more specific patterns above (e.g. bare "founder",
        // "founder of smartpark", "who made this", "who built this", "creator?").
        if (lowerMsg.includes('founder') || lowerMsg.includes('creator') ||
            (lowerMsg.includes('who') && (lowerMsg.includes('created') || lowerMsg.includes('built') || lowerMsg.includes('made') || lowerMsg.includes('developed'))) ||
            (lowerMsg.includes('developer') && !lowerMsg.includes('developer mode'))) {
            return 'about_founder';
        }
        
        // ===== GENERAL SUPPORT DETECTION =====
        if (/^(support|help|helpline|customer care|customer service)$/i.test(lowerMsg.trim()) ||
            (lowerMsg.includes('support') && !lowerMsg.includes('phone') && !lowerMsg.includes('email') && !lowerMsg.includes('call'))) {
            return 'support_general';
        }
        
        // ===== DASHBOARD / SECURITY (previously had no detection path) =====
        if (has('dashboard')) {
            return 'dashboard_features';
        }
        if (hasAny(['guard', 'guards', 'patrol', 'patrolling'])) {
            return 'security_guards';
        }
        
        // ===== LOST VEHICLE / FORGOT SLOT (checked before availability/booking =====
        // ===== so "forgot" doesn't get swallowed by forgot_password etc.) =====
        if (hasAny(['forgot', 'remember', 'which']) && has('slot')) {
            return 'forgot_slot';
        }
        if (hasAny(['lost', 'locate', 'find']) && hasAny(['car', 'vehicle'])) {
            return 'find_car';
        }
        
        // ===== FLOOR-SPECIFIC DETECTION (checked before generic availability) =====
        if (has('ground') && hasAny(['floor', 'level'])) {
            return 'ground_floor';
        }
        if ((has('first') && hasAny(['floor', 'level'])) || has('level 1') || has('floor 1')) {
            return 'first_floor';
        }
        if ((has('second') && hasAny(['floor', 'level'])) || has('top floor') || has('level 2') || has('floor 2')) {
            return 'second_floor';
        }
        
        // ===== EV SUB-TOPIC DETECTION (ordered: specific sub-topics first, =====
        // ===== bare price/general EV mention falls through to ev_price last) =====
        if (hasAny(['ev', 'electric']) && hasAny(['where', 'location', 'locations', 'find'])) {
            return 'ev_locations';
        }
        if (hasAny(['ev', 'electric', 'charging']) && hasAny(['how long', 'duration', 'minutes', 'time to charge'])) {
            return 'ev_charging_time';
        }
        if (hasAny(['ev', 'electric']) && has('discount')) {
            return 'ev_discount';
        }
        
        // ===== GENERAL AVAILABILITY DETECTION =====
        if (lowerMsg.includes('available spot') || lowerMsg.includes('available slot') ||
            lowerMsg === 'available' || lowerMsg.includes('any slot') ||
            lowerMsg.includes('is there parking') || lowerMsg.includes('open spot')) {
            return 'available_now';
        }
        
        // ===== GENERAL BOOKING DETECTION =====
        if ((lowerMsg.includes('how to book') || lowerMsg.includes('how do i book') ||
            lowerMsg.includes('how can i book') || lowerMsg.includes('book a slot') ||
            lowerMsg.includes('book a spot') || lowerMsg === 'book' || lowerMsg.includes('make a booking')) &&
            !has('extend')) {
            return 'how_to_book_general';
        }
        
        // ===== EXTEND SUB-TOPIC DETECTION (checked before cancel/booking so =====
        // ===== "extend" questions don't get swallowed by those) =====
        if (has('extend') && hasAny(priceWords)) {
            return 'extend_cost';
        }
        if (has('extend') && hasAny(['limit', 'max', 'maximum', 'longest'])) {
            return 'extend_limit';
        }
        if (has('extend')) {
            return 'how_to_extend';
        }
        
        // ===== PROMO SUB-TOPIC DETECTION =====
        if (has('promo') && hasAny(['apply', 'use', 'enter', 'redeem'])) {
            return 'apply_promo';
        }
        if (has('promo')) {
            return 'current_promos';
        }
        
        // ===== RECEIPT SUB-TOPIC DETECTION =====
        if (has('receipt') && hasAny(['email', 'send'])) {
            return 'email_receipt';
        }
        
        // ===== CANCELLATION SUB-TOPIC DETECTION (specific reasons first, =====
        // ===== bare "cancel"/"cancellation" mention falls through to how_to_cancel last) =====
        if (hasAny(cancelWords) && hasAny(['deadline', 'how long', 'until when', 'last minute'])) {
            return 'cancel_deadline';
        }
        if (hasAny(cancelWords) && hasAny(['online', 'myself', 'website', 'app'])) {
            return 'cancel_online';
        }
        if (hasAny(cancelWords) && has('refund')) {
            return 'cancel_refund';
        }
        if (hasAny(cancelWords) && hasAny(['policy', 'rules', 'terms'])) {
            return 'cancel_policy';
        }
        if (has('cancel') || has('cancellation')) {
            return 'how_to_cancel';
        }
        
        // ===== PRICING SUB-TOPIC DETECTION (specific tier first, bare EV/ =====
        // ===== general pricing mention falls through to catch-alls last) =====
        if (hasAny(['standard', 'regular', 'normal']) && hasAny(priceWords)) {
            return 'standard_price';
        }
        if (has('premium') && (hasAny(priceWords) || has('parking'))) {
            return 'premium_price';
        }
        if (hasAny(['night', 'overnight']) && (hasAny(priceWords) || has('parking') || has('after 8pm'))) {
            return 'night_rate';
        }
        if (hasAny(['weekend', 'saturday', 'sunday']) && (hasAny(priceWords) || has('parking'))) {
            return 'weekend_rate';
        }
        if (has('early bird') || (has('early') && hasAny(priceWords)) || has('before 8am')) {
            return 'early_bird';
        }
        // Any remaining EV + price mention (locations/time/discount already handled above)
        if (hasAny(['ev', 'electric']) && (hasAny(priceWords) || has('cost'))) {
            return 'ev_price';
        }
        
        // ===== GENERAL PRICING DETECTION (catches broader phrasing) =====
        if (lowerMsg.includes('how much') || lowerMsg.includes('pricing') ||
            lowerMsg.includes('cost') || lowerMsg.includes('rates') ||
            lowerMsg.includes('price') || lowerMsg.includes('plans')) {
            return 'pricing_general';
        }
        
        // ===== GENERAL KEYWORD MAPPING =====
        // Uses word-boundary matching (\b) instead of plain substring search.
        // Plain .includes() caused false positives like "support" matching the
        // short keyword "sup" (intended only for standalone "sup" as slang for
        // "what's up"). Escaping + \b keeps multi-word phrases working exactly
        // as before while fixing short-keyword false matches.
        ...
        // ===== FINAL FALLBACK: TOPIC WORD MATCHING =====
        // Everything above requires an exact phrase/keyword match. Real users
        // phrase things in many ways ("I want to reserve a slot", "what's the
        // fee", "is there a camera here", "how do I sign up", "any discount
        // codes"), which don't match a fixed phrase list. This scores the
        // message against topic word-groups and returns the best match
        // instead of falling straight to the generic default response.
        const topicWordGroups = {
            'how_to_book_general': ['book', 'booking', 'reserve', 'reservation', 'reserving'],
            'pricing_general': ['price', 'prices', 'pricing', 'cost', 'costs', 'rate', 'rates', 'fee', 'fees', 'charge', 'charges'],
            'how_to_cancel': ['cancel', 'cancellation', 'cancelling'],
            'available_now': ['available', 'availability', 'vacant', 'empty', 'spot', 'spots', 'slot', 'slots'],
            'ev_price': ['ev', 'electric', 'charging', 'charger'],
            'cctv': ['cctv', 'camera', 'cameras', 'surveillance', 'monitored', 'safe', 'safety'],
            'create_account': ['account', 'signup', 'register', 'registration', 'login', 'signin'],
            'forgot_password': ['password', 'forgot'],
            'get_receipt': ['receipt', 'invoice', 'bill'],
            'how_to_extend': ['extend', 'extension', 'extending'],
            'handicap_spots': ['handicap', 'disabled', 'wheelchair', 'accessible', 'accessibility'],
            'find_car': ['lost', 'locate', 'find', 'forgot'],
            'current_promos': ['promo', 'promos', 'coupon', 'discount', 'code', 'offer', 'offers'],
            'office_hours': ['office', 'address', 'location', 'timings', 'hours'],
            'phone_support': ['phone', 'call', 'number'],
            'email_support': ['email', 'mail'],
            'about_founder': ['founder', 'creator', 'owner'],
            'support_general': ['support', 'help', 'assistance', 'problem', 'issue'],
            'about_smartpark': ['smartpark', 'app', 'website', 'platform', 'website']
        };

        const messageWords = lowerMsg.replace(/[^\w\s]/g, ' ').split(/\s+/).filter(Boolean);
        let bestType = null;
        let bestScore = 0;

        for (const [type, words] of Object.entries(topicWordGroups)) {
            let score = 0;
