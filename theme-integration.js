...
(function() {
    ...
    function loadUserTheme() {
        ...
        if (savedTheme && themeManager.themes[savedTheme]) {
            // notify=false: this just restores whatever theme was already
            // active on the previous visit — it's not a new choice the
            // person just made, so it shouldn't pop a "Theme changed" toast
            ...
            if (appState && appState.currentUser) {
                ...
                if (email.includes('business') || email.includes('enterprise') || email.includes('corp')) {
                    themeManager.setTheme('enterprise-blue', false);
    ...
    function addThemeToggleToNavbar() {
        // theme.js already creates the primary theme toggle button
        // (#theme-toggle-btn) in the navbar during its own init. Skip adding
        // a second one here to avoid two palette buttons showing up side by side.
    ...
    function applySeasonalTheme() {
        if (!themeManager.preferredTheme || themeManager.preferredTheme === 'auto') {
            ...
            // These run automatically on every load/interval check, not
            // from a person clicking anything — so notify=false, otherwise
            // a "Theme changed" toast would pop up unprompted.
            
            // Christmas (December)
            if (month === 12) {
                themeManager.setTheme('christmas', false);
                return;
            }
            
            // Halloween (October 15-31)
            if (month === 10 && day >= 15) {
                themeManager.setTheme('halloween', false);
                return;
            }
            
            // Summer (June-August)
            if (month >= 6 && month <= 8) {
                themeManager.setTheme('summer', false);
                return;
            }
            
            // Winter (November-February, excluding December)
            if ((month === 11 || month <= 2) && month !== 12) {
                themeManager.setTheme('winter', false);
                return;
            }
        }
    }
        ...
        function checkTimeForDarkMode() {
            ...
            if (isNight && themeManager.preferredTheme === 'auto') {
                ...
                if (!localStorage.getItem(`${config.storagePrefix}manual_override`)) {
                    themeManager.setTheme('dark', false); // automatic — no toast
                }
            }
        }
